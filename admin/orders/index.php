...
<div class="flex items-end justify-between gap-6 flex-wrap">
    <div>
        ...
        <h1 class="mt-3 font-display text-3xl md:text-4xl">Bestellingen</h1>
        <p class="mt-3 text-white/70">Alle binnenkomende bestellingen in één overzicht.</p>
    </div>
...
<div class="mt-6 grid gap-3">
    <?php if (!$orders): ?>
        <div class="rounded-[24px] p-6 luxe-ring bg-black/25">
            <div class="text-sm text-white/70">Geen bestellingen gevonden voor deze filters.</div>
            <p class="mt-2 text-xs text-white/50">Pas de filters aan om andere resultaten te zien.</p>
        </div>
    <?php endif; ?>

    <?php foreach ($orders as $order): ?>
        <div class="rounded-[24px] overflow-hidden luxe-ring bg-black/25 shadow-glow">
            <div class="p-4 md:p-5 grid gap-4 md:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_auto]">
                <div class="space-y-1">
                    <div class="flex flex-wrap items-center gap-2 text-[11px] tracking-[.28em] uppercase text-white/50">
                        <span>Bestelling #<?= h((string)$order['id']) ?></span>
                        <span class="text-white/30">•</span>
                        <span class="text-white/40"><?= h($order['order_token']) ?></span>
                    </div>
                    <div class="font-display text-lg md:text-xl gold-text">
                        <?= h($order['first_name'] . ' ' . $order['last_name']) ?>
                    </div>
                    <div class="text-xs text-white/60"><?= h($order['email']) ?></div>
                </div>

                <div class="grid grid-cols-2 gap-3 text-xs md:text-sm">
                    <div>
                        <div class="text-white/50">Totaal</div>
                        <div class="text-base md:text-lg text-white font-medium">€ <?= number_format((float)$order['total'], 2, ',', '.') ?></div>
                        <div class="text-[11px] text-white/40"><?= h($order['currency']) ?> • <?= (int)$order['item_count'] ?> items</div>
                    </div>
                    <div>
                        <div class="text-white/50">Datum</div>
                        <div class="text-sm text-white/80"><?= h($order['created_at']) ?></div>
                    </div>
                </div>
